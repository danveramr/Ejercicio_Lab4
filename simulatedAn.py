...
for i in range(100):
    for j in range(100):
        ...
        for k in range(1):
            solucion_actual[k] = mejor_sol[k] + 0.1*(random.uniform(lim_i[k],lim_s[k]))
            solucion_actual[k] = max(min(solucion_actual[k],lim_s[k]),lim_i[k])
        
        posible_sol = funcion(solucion_actual)
